...
# load data
x_list = torch.load('X_list.pt')  # 28 * [4, 700, 150]
...
# 有n_samples=28个样本点，每个样本点的shape为4*700*150  bsz x d_model x qlen
n_samples, bsz, d_model, qlen = x_traj.shape[0], x_traj.shape[1], x_traj.shape[2], x_traj.shape[3]
...
ct = ConsistencyFunction(nfeat, nfeat).cuda()
...
# save model
torch.save(ct.state_dict(), 'ct.pt')

# load model
ct = ConsistencyFunction(nfeat, nfeat).cuda()
ct.load_state_dict(torch.load('ct.pt'))
