...
def _build_cm_refiner(config):
    dataset = config.CM.DATASET or config.DATASET.DATASET
    cm_config = CMPluginConfig(
        enabled=config.CM.ENABLE,
        dataset=dataset,
        task=config.CM.TASK,
        base_dir=config.CM.SAVE_DIR,
        load_path=config.CM.LOAD_PATH or None,
        save_dir=config.CM.SAVE_DIR,
    )
    return build_cm_refiner(cm_config)
...
def train(config, train_loader, model, criterion, optimizer, lr_scheduler, epoch,
    ...
    model.train()
    cm_refiner = _build_cm_refiner(config)
    ...
    with torch.no_grad():
        ...
        for i, (input, target) in enumerate(val_loader):
            ...
            output, _, sradius = model(input, 
                                 ...
                                 writer=writer, cm_refiner=cm_refiner)
            target = target.cuda(non_blocking=True)
            loss = criterion(output, target)
